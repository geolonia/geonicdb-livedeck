...
- **G / ☰ ボタン / ページ番号のクリック**: 目次を開く。全ページを章（本編・Live Demo・Appendix・用語集）ごとに一覧し、**任意のページへ直接ジャンプ**できる。項目のクリック、または ↑ ↓ ← → で選んで Enter。**Esc / ✕ / 背景クリック**で閉じる。開いている間は自動再生を一時停止し、閉じると同じページから再開する
- **F**: 全画面　**Esc**: 解除
