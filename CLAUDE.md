...
## 会場の一斉アクセス（認証ハンドシェイクのジッタ／リトライ）

会場投稿デモは参加者の端末が**同じ瞬間に**同じ URL を開く。SDK の API キー認証は
1 端末あたり `POST /auth/nonce` → PoW → `POST /oauth/token` の 2 往復を必ず踏み、
どちらも GeonicDB の**制御プレーン**（`/auth` `/oauth` 専用 Lambda）に当たるため、
一斉アクセスはここで詰まる。

**実測（2026-09-02 / staging・1 IP から `/auth/nonce` を 200 並列）**: 500 が 93 件
（Lambda スロットル。クライアントへは 429 ではなく `InternalServerErrorException`）、
429 が 72 件（送信元 IP 単位のレート制限。会場 Wi-Fi では**全員が 1 つのバケットを共有**する）、
成功相当が 35 件。**17.5% しか通らない。** かつ SDK は nonce / token 取得の失敗を
リトライしない（`AuthenticationError` を投げて終わる）。

そのため `src/lib/authGate.ts` を必ず通す:

- `waitAuthJitter()` — 最初のリクエストを 0〜8 秒ランダムに遅らせて波を平らにする
- `withAuthRetry()` — 429 / 5xx / ネットワーク断だけ指数バックオフ＋ジッタでやり直す
  （400/401/403 は 1 回で諦める。何度やっても同じで制御プレーンを余計に詰まらせる）

**新しく会場参加型のデモを足すときは、SDK クライアントの最初の呼び出しを必ずこの 2 つで包む。**
発表者の画面だけ即座に出したい場合は URL に `?nojitter` を付ける（リトライは残る）。
上限は `VITE_AUTH_JITTER_MS` で変えられる。

なお**サーバ側の上限もセットで見る**こと（geonicdb 本体 `docs/QUOTAS.md` の
「共有 NAT（`PUBLIC_RATE_LIMIT_CLIENTS_PER_IP`）」）。ジッタだけでは
IP 単位の毎分上限は越えられない。
